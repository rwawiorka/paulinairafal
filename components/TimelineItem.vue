<template>
  <div
    ...
    :class="{ 'flex': reverse, 'justify-end': reverse }">
    <!-- Icon -->
    <div
    ...
    <div class="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] p-4">
      ...
      <i v-else class="icon-timeline" :class="[icon, { 'flex': reverse, 'justify-end': reverse }]"></i>
      <div class="font-bold text-slate-900 text-2xl" :class="{ 'flex': reverse, 'justify-end': reverse }">{{ title }}
      </div>
      <time v-if="time" class="font-caveat font-medium text-slate-900 text-2xl"
        :class="{ 'flex': reverse, 'justify-end': reverse }">{{ time }}</time>
      <div v-if="location" class="text-slate-500 text-xl" :class="{ 'flex': reverse, 'justify-end': reverse }">{{
      location }}</div>
    </div>
  </div>
</template>
